...
import { InboxIcon, LinkIcon, UserIcon, ClockIcon, CheckCircleIcon, EyeIcon, ImageIcon, Maximize2Icon } from "lucide-react"
...
export function PostList({ status }: PostListProps) {
  ...
  async function fetchPosts(pageNumber: number, shouldAppend = true) {
    ...
    try {
      const response = await fetch(
        `/api/reaction/posts?status=${status}&page=${pageNumber}&limit=10`
      )
  ...
  async function handleVisualize(post: Post) {
    ...
    toast.success("Conteúdo marcado como visualizado!")

    try {
      const response = await fetch(`/api/reaction/posts/${post.id}`, {
        method: "PATCH",
        headers: {
    ...
    return (
      <Card className="p-12 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-muted-foreground">
          {status === "approved"
            ? "Não há conteúdos aprovados para visualizar."
            : "Não há conteúdos visualizados no momento."}
        </p>
      </Card>
    )
  }
              ...
              <div className="space-y-4 w-full">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4 text-sm text-muted-foreground">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{post.username || "Nome não informado"}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {status === "approved" ? (
                      <>
                      ...
                      <>
                        ...
                        <span>
                          Visualizado {formatDistanceToNow(new Date(post.updatedAt), {
                            addSuffix: true,
                            locale: ptBR,
                          })}
                        </span>
